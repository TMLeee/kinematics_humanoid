...
## 11. base 추정 / 접촉 pose servo / 어드미턴스 / 게걸음 (branch `change_frame`, 2차)

### 11.1 base pose = 접지발 고정 가정의 kinematic odometry (`BaseEstimator`)

`src/controller/BaseEstimator.h` 신설. 발마다 **접지 순간의 world pose 를 anchor** 로 잡고,
매 tick `R_base = R_anchor·R_footInBase^T`, `p_base = p_anchor − R_base·p_footInBase` 로 역산한다.

- 기존과의 차이: anchor 를 **계획 포즈로 매 tick 스냅**하지 않고 **직전 추정에서 물려받는다**
  → 지지 교체 때 base 가 점프하지 않는다(기존은 스텝마다 스윙발 추종오차만큼 점프).
- 드리프트: 한 지지구간 안에서는 FK 정확(적분 없음). 스텝 간에도 **누적되지 않는다** —
  스윙발 목표가 절대 world 계획 포즈이므로 `anchor_n = plan_n + (그 스텝 추종오차)` 로
  매 스텝 리셋된다. 실측 `anchorVsPlan` rms **0.13 mm**, max **0.15 mm** (15 s 전진).
- 평지 가정: anchor 의 z 와 roll/pitch 는 접지 평면으로 투영하고 x/y/yaw 만 누적한다.
  이 가정이 §2 의 프레임 오차(최대 113 mm)의 주범이며, **그것을 대체하는 자리가
  `applyCorrection()`** 이다.
- **IMU / SLAM 인터페이스**: `BaseEstimator::Correction { attitude, yaw, position + weight }`.
  base pose 는 anchor 에서 유일하게 결정되므로 base 보정 = anchor 보정이고,
  `applyCorrection()` 이 그 변환(관측 base pose → anchor)을 해 준다. IMU 자세만 주면
  anchor 의 roll/pitch 가 실측 기울기로 대체된다(§8 의 `R_sole_true` 와 같은 식).
  yaw 는 관측에서 취하지 않는다(드리프트 + 계획 heading 과 어긋남). 호출 지점은
  `HumanoidController::update()` 의 `solveBase` 직전에 주석으로 표시해 두었다. 현재 호출자 없음.
- `comWithPlantedFoot()` 도 같은 anchor·tilt 를 쓴다 → 측정 COM 과 내부 모델 프레임이 항상 일치.
- 되돌리기: `baseAnchorPlan=1`.

**순서가 중요하다**: 새로 접지한 발의 anchor 는 **base 를 푼 뒤** 그 tick 의 모델에서 잡아야
한다. 직전 tick 의 base 로 잡으면 한 tick 어긋나 착지 tick 에 과도항이 남는다
(실측: `conPoseErr` max 5.67e-4 → **1.28e-4**, `anchorVsPlan` 0.56 → **0.15 mm**).

### 11.2 접촉 구속을 pose servo 로 — 지지발에 따른 구조 변화 제거

접촉 task 의 목표를 속도 0 에서 **anchor 로의 6D pose servo** (`xdot = kpContact·e`) 로 바꿨다.
자코비안은 `[지지발; 반대발]` 순서로 항상 12행을 만들어 두고 단일지지면 위 6행만 쓴다
→ **지지발이 바뀌어도 스택 구조가 그대로다.**

- 지지발의 정상 상태 오차는 **정확히 0** 이다(base 추정이 발을 anchor 에 놓으므로) → servo 무해.
- 값이 생기는 경우는 두 가지뿐: (1) DS 에서 반대발이 자기 anchor 에서 어긋남 → **위치레벨**
  폐루프 정합(기존 양발 구속은 속도레벨만), (2) 어드미턴스가 anchor 자세를 기울임 → §11.3.
- `kpContact` 실측: max 오차는 kp 와 무관(접촉 전이가 정하는 값)이고 rms 만 kp 로 줄어든다
  (kp 6/20/60 → rms 3.4e-5 / 2.0e-5 / 1.2e-5). 기본 20 (어드미턴스 1차 지연 20 rad/s 와 정합).
- COM/양발 모두 접촉 구속의 null space 안에서 제어된다(이미 §10 에서 확인, 구조 변경 없음).

### 11.3 발목 어드미턴스 — **적용 지점을 옮기니 동작한다**

순응 회전을 **발 목표 자세와 anchor 에 동시에** 곱한다(`ankleAdmAtFootTarget=1`):
지지발은 `anchor·tilt`(base 추정·접촉 task·`comWithPlantedFoot` 전부 같은 값), 스윙발은 `Rdes·tilt`.
→ 내부 모델·base 추정·골반 태스크가 모두 같은 "발이 δ 기울었다" 를 보므로 **싸우지 않고**,
IK 가 순응을 다리 전체로 실행한다. §6 이 처방한 그대로다.

**부호는 −1 이다**(`ankleAdmSign=-1`). 관절 덧셈과 발 자세 회전은 몸통에 반대로 작용한다.
실측(15 s × 5 조건):

| 어드미턴스 | 전도 | 비고 |
|---|---|---|
| off | 0/5 | 기준 |
| 발 목표, 부호 **−1** | **0/5** | ZMP 여유 개선 ↓ |
| 발 목표, 부호 +1 | 5/5 | 부호가 반대 |
| 출력단 관절 덧셈(기존) | 3/5 | §6 재현 |

지지발 대비 ZMP 여유 최대치(복제쌍 2회):

| 조건 | off | 발 목표 −1 |
|---|---|---|
| vx=0.04 | 0.138 ~ 0.145 | **0.095 ~ 0.107** (−27%) |
| vx=0.06 (=vFwdMax) | 0.133 | **0.117 ~ 0.126** (−8%) |
| Tstep=0.7 | 0.407 ~ 0.435 | **0.351 ~ 0.354** (−16%) |

복제쌍 노이즈(±5%)를 넘는 개선이고 5/5 조건 같은 방향이다. 기본 **on**.
한계: `vFwdMax` 를 넘는 vx=0.08 에서는 복제쌍 1/2 전도(off 는 2/2 생존).
그 영역까지 쓰려면 `ankleAdmKPitch` 를 0.025 로 낮춘다(2/2 생존, 대신 이득 소멸).

주의: 어드미턴스가 켜지면 `slip ang` 과 `conPoseErr` 이 커지는 것이 정상이다 —
그건 위반이 아니라 **의도된 순응 회전**이다(각속도 9.2e-2 rad/s).

### 11.4 게걸음 발 겹침 — 원인과 처방

계획 기준 두 발 중심 최소 간격을 재면 정확히 **`2·halfWidth − vy·T = 0.205 − vy·T`** 다.
발 반폭×2 = 0.130 이 닿는 값이므로 **vy > 0.075 m/s 에서 계획이 이미 겹친다**(실측:
vy=0.08 → 0.125). 원인은 매 스텝 앵커가 dy 씩 옆으로 가는데 두 발이 그 앵커에서 ±halfWidth
로 놓여서, **진행 반대쪽 발**이 착지할 때만 간격이 dy 만큼 줄어드는 비대칭이다.

두 가지 처방을 각각 실측했다(실측 발 위치는 전도 후 자세가 섞여 원인/결과를 못 가르므로
**계획 기준** 간격으로 판정했다):

| 처방 | 계획 최소 간격 | 최대 벌림 | 결과 |
|---|---|---|---|
| 없음 | 0.205 − vy·T (vy=0.08 → 0.125) | 0.225 | vy>0.075 에서 겹침 |
| `sidestepLeadOnly=1` (요청안: 반대발은 진행방향으로 안 움직임) | **0.205 고정** | **0.325** | vy≥0.06 에서 복제쌍 **2/2 전도** |
| `minFootClearance=0.145` (안전망) | **0.145** | 0.305 | vy=0.08 복제쌍 **2/2 생존** |

`sidestepLeadOnly` 는 겹침을 완전히 없애지만 진행쪽 발이 2·dy 를 한 번에 벌리므로 최대
벌림이 두 배가 되고(0.225 → 0.325 m) 개루프 보행이 그 측방 체중이동을 못 버틴다.
안전망은 부족분만 밀어내므로 벌림 증가가 작고 안정성 손실이 없다.
→ **기본값: `sidestepLeadOnly=0`, `minFootClearance=0.145`.** 요청안은 노브로 남겼다.

(부수 확인: vy=0.02 에서 겹침 없이(0.185) 전도한 사례는 복제쌍이 생존/전도로 갈렸다
→ §10.6 의 카오스. 겹침과 무관.)
...
| 검증 | 결과 |
|---|---|
| `floatingBase=0 pelvisBodyFrame=0 baseAnchorPlan=1 ankleAdmEnable=0` | dx **+0.6058**, SS slip 2.761e-07, DS slip 1.134e-03 — 원본과 **비트 단위 일치** |
| 정식화 동치성(λ=1e-8, 단일접촉) | 달성 task 속도 차 COM 1.3e-14 m/s |
| 골반 좌표 동치성(`ikCompare=2`) | dq 상대차 1.07e-15 |
| 게걸음 vy=0.04 / 회전 vyaw=0.15 | 계획 간격 0.165 / 0.199, 전도 없음 |
| 기본값 15 s 전진 | dx +0.606, 전도 없음, ZMP 여유 x **0.095**(어드미턴스 off 대비 −36%) |

### 11.6 다음

§9 의 1번(재앵커링 `R_pl` 에 IMU 실측 기울기)은 이제 **`BaseEstimator::applyCorrection()` 호출
한 줄**이 되었다. 다만 §10.7 이 보여준 대로 측정 정확도만 올리면 오히려 나빠질 수 있으므로
DCM/캡처포인트 형태의 감쇠 있는 피드백과 함께 넣어야 한다.
